// avoid duplicate code for connecting to mongoose
const mongoose = require('mongoose')
const amqp = require("amqplib")
const messageQueueListener = require("./src/MessageListener/RabbitQueueListener");

// these options are to not let mongoose use deprecated features of the mongo driver
const options = {
    useNewUrlParser: true,
    useUnifiedTopology: true,
}

async function mongo(dbName) {
    try {
        await mongoose.connect(`${process.env.MONGO_URL}/${dbName}?authMechanism=DEFAULT&authSource=admin`, options)
        console.log(`connection to mongo DB ${dbName} established`)
    } catch (err) {
        console.error(err)
    }
}

async function rabbitMq(){
    const connection = await amqp.connect(process.env.RABBITMQ_URL)
    const channel = await connection.createChannel();

    const ratingQueue = "ratingQueue"
    await channel.assertExchange(process.env.RABBITMQ_EXCHANGE)
    channel.assertQueue(ratingQueue, {durable:true})
    channel.bindQueue(ratingQueue, process.env.RABBITMQ_EXCHANGE, "mealKey")
    channel.bindQueue(ratingQueue, process.env.RABBITMQ_EXCHANGE, "userKey")

    //Handlers
    await messageQueueListener.listen(channel)
    console.log(`Connected to RabbitMq Instance ${process.env.RABBITMQ_URL} with exchange ${process.env.RABBITMQ_EXCHANGE}`)
}

module.exports = {
    mongo,
    rabbitMq
}
